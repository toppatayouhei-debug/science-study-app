...
if sub == "システム英単語":
    ...
    all_cats = df_raw["category"].dropna().unique().tolist()

    priority_cats = [
        c for c in all_cats
        if any(kw in str(c) for kw in ["微分", "積分", "極限"])
    ]
...
if sub == "システム英単語":
    ...
    for i, val in enumerate(st.session_state.choices):
        ...
        with c1 if i % 2 == 0 else c2:
            ...
            if st.button(
                val,
                key=f"t_{st.session_state.idx}_{i}",
