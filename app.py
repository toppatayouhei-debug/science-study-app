...
.card { background-color: white !important; padding: 20px !important; border-radius: 12px !important; box-shadow: 0 4px 10px rgba(0,0,0,0.08) !important; }
...
if sub == "システム英単語":
    ...
    filter_label = st.sidebar.radio("レベル", list(lv_map.keys()))
    ...
    filter_label = st.sidebar.radio("分野", ["すべて"] + all_cats)
    current_filter_val = filter_label
    filter_col = "category"

# 【修正ポイント】セッション状態の更新検知
if "current_sub" not in st.session_state or st.session_state.current_sub != sub or st.session_state.get("last_filter_label") != filter_label:
    st.session_state.current_sub = sub
    st.session_state.last_filter_label = filter_label # ラベルで比較
    
    # フィルタリング実行
    if filter_label == "すべて":
        df_f = df_raw.copy()
    else:
        df_f = df_raw[df_raw[filter_col].astype(str) == current_filter_val]
...
if sub == "システム英単語":
    ...
    if "choices" not in st.session_state:
        ...
        choices = [correct] + random.sample(dummy_pool, min(3, len(dummy_pool)))
    ...
    for i, val in enumerate(st.session_state.choices):
        with (c1 if i % 2 == 0 else c2):
            if st.button(val, key=f"b_{i}", disabled=st.session_state.answered):
